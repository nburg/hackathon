...
var DEFAULT_SKIP_ELEMENTS = [
  ...
  "option",
  "nav",
  // Add nav element to skip list
  "a"
  // Skip links
];
var DEFAULT_SKIP_ARIA_ROLES = [
  "navigation",
  "banner",
  "complementary",
  "contentinfo",
...
function shouldSkipNodeOrAncestor(node, config) {
  let current = node;
  while (current) {
    if (current.nodeType === 1) {
      if (shouldSkipElement(current, config)) {
        return true;
      }
    }
...
function extractTextNodes(root = document, config = {}) {
  const textNodes = [];
...
function extractContentText(root = document, config = {}) {
...
// src/nlp/tagger.ts
function mapCompromiseTag(tags) {
  ...
  jsonTerms.forEach((termGroup) => {
    if (termGroup.terms && termGroup.terms.length > 0) {
      termGroup.terms.forEach((term) => {
        ...
        const offset2 = text.indexOf(wordText, currentOffset);
        if (offset2 >= 0) {
          results.push({
            text: wordText,
            pos,
          ...
          currentOffset = offset2 + wordText.length;
        }
      });
    }
  });
...
function detectMultiWordExpressions(text) {
  const doc = three_default(text);
  const multiWords = [];
  doc.nouns().forEach((noun) => {
    const phrase = noun.text();
    if (phrase && phrase.includes(" ")) {
      const start2 = text.indexOf(phrase);
      if (start2 >= 0) {
        multiWords.push({
          start: start2,
          end: start2 + phrase.length,
          phrase
        });
      }
    }
...
function isWithinMultiWord(offset2, multiWords) {
  return multiWords.some((mw) => offset2 >= mw.start && offset2 < mw.end);
}
function filterContentWords(taggedWords) {
...
// src/extractor.ts
function extractCandidates(root = document, config = {}) {
  const candidates = [];
  ...
  textNodes.forEach((node) => {
    ...
    const multiWords = config.nlp?.detectMultiWord !== false ? detectMultiWordExpressions(text) : [];
...
function extractCandidatesFromElement(element, config = {}) {
  return extractCandidates(element, config);
}
function filterCandidates(candidates, filter2) {
  return candidates.filter(filter2);
}
function groupCandidatesByElement(candidates) {
...
function getSentenceForCandidate(candidate) {
